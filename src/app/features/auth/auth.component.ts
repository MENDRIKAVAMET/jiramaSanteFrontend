  ...
  template: `
    <div class="auth-container">
      <mat-card class="auth-card">
        ...
        <p class="auth-footer">
          Application de déclaration sanitaire — Agents JIRAMA
        </p>
      </mat-card>
    </div>
  `,
...
export class AuthComponent {
  ...
  onSubmit(): void {
    ...
    this.auth.login({ email, password }).subscribe({
      next: (session) => {
        ...
        const target = redirectUrl ?? getDefaultRouteForRole(session.user.role);
  ...
  private getRedirectUrl(): string | null {
    ...
    return redirect && redirect.startsWith('/') && redirect !== '/login' ? redirect : null;
  }
}
